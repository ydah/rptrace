...
RSpec.describe Rptrace::Tracee do
  describe "instance controls" do
    ...
    before do
      ...
      tracee.instance_variable_set(:@registers, instance_double(Rptrace::Registers))
      tracee.instance_variable_set(:@memory, instance_double(Rptrace::Memory))
    end

    it "sends CONT request and returns self" do
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_CONT, 4321, 0, 9)
      expect(tracee.cont(signal: 9)).to eq(tracee)
    end

    it "sends SYSCALL request and returns self" do
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_SYSCALL, 4321, 0, 0)
      expect(tracee.syscall).to eq(tracee)
    end

    it "sends SINGLESTEP request and returns self" do
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_SINGLESTEP, 4321, 0, 2)
      expect(tracee.singlestep(signal: 2)).to eq(tracee)
    end

    it "sends INTERRUPT request and returns self" do
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_INTERRUPT, 4321, 0, 0)
      expect(tracee.interrupt).to eq(tracee)
    end

    it "sends LISTEN request and returns self" do
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_LISTEN, 4321, 0, 0)
      expect(tracee.listen).to eq(tracee)
    end

    it "sets ptrace options and returns self" do
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(
        Rptrace::Constants::PTRACE_SETOPTIONS,
        4321,
        0,
        7
      )
    ...
    it "enables seccomp event tracing options" do
      expect(tracee).to receive(:set_options).with(
        Rptrace::Constants::PTRACE_O_TRACESYSGOOD | Rptrace::Constants::PTRACE_O_TRACESECCOMP
      ).and_return(tracee)
    ...
    it "sends DETACH request and returns self" do
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_DETACH, 4321, 0, 0)
    ...
    it "wraps waitpid result in Event" do
      allow(Rptrace::Binding).to receive(:safe_waitpid).with(4321, flags: 7).and_return([4321, 0x2A00])
    ...
    it "raises unsupported arch error for unknown syscall layout" do
      expect { tracee.current_syscall(arch: :mips) }.to raise_error(Rptrace::UnsupportedArchError)
    end

    it "returns parsed /proc memory maps for pid" do
      maps = [Rptrace::ProcMaps::Mapping.new(
        start_addr: 0x1000,
        end_addr: 0x2000,
        permissions: "r--p",
        offset: 0,
      ...
      expect(Rptrace::ProcMaps).to receive(:read).with(4321).and_return(maps)
    ...
    it "reads ptrace event message via GETEVENTMSG" do
      expected = 123_456
      allow(Rptrace::Binding).to receive(:safe_ptrace) do |request, pid, addr, data|
        expect(request).to eq(Rptrace::Constants::PTRACE_GETEVENTMSG)
    ...
    it "reads seccomp metadata via ptrace helper" do
      allow(Rptrace::Binding).to receive(:safe_ptrace) do |request, pid, addr, data|
        expect(request).to eq(Rptrace::Constants::PTRACE_SECCOMP_GET_METADATA)
        expect(pid).to eq(4321)
        expect(addr).to eq(Rptrace::CStructs.seccomp_metadata_size)
        pointer = Fiddle::Pointer.new(data)
        decoded = Rptrace::CStructs.unpack_seccomp_metadata(pointer[0, Rptrace::CStructs.seccomp_metadata_size])
        expect(decoded[:filter_off]).to eq(2)
        pointer[0, Rptrace::CStructs.seccomp_metadata_size] = Rptrace::CStructs.pack_seccomp_metadata(
          filter_off: 2,
          flags: 0x80
        )
        0
    ...
    it "reads and decodes seccomp filter instructions" do
      ...
      allow(Rptrace::Binding).to receive(:safe_ptrace) do |request, pid, addr, data|
        expect(request).to eq(Rptrace::Constants::PTRACE_SECCOMP_GET_FILTER)
        expect(pid).to eq(4321)
        expect(addr).to eq(1)
    ...
    it "returns empty seccomp filter when tracee has no filter instructions" do
      allow(Rptrace::Binding).to receive(:safe_ptrace).with(
        Rptrace::Constants::PTRACE_SECCOMP_GET_FILTER,
        4321,
        0,
        0
      ).and_return(0)
    ...
    it "returns false when seccomp metadata query is unsupported" do
      allow(tracee).to receive(:seccomp_metadata).with(index: 0).and_raise(Rptrace::InvalidArgError.new("unsupported"))
    ...
    it "returns false when seccomp filter query is unsupported" do
      allow(Rptrace::Binding).to receive(:safe_ptrace).with(
        ...
        1,
        0
      ).and_raise(Rptrace::InvalidArgError.new("unsupported"))
    ...
    it "decodes known seccomp metadata flag names" do
      allow(tracee).to receive(:seccomp_metadata).with(index: 0).and_return(
        filter_off: 0,
        flags: Rptrace::Constants::SECCOMP_FILTER_FLAG_TSYNC | Rptrace::Constants::SECCOMP_FILTER_FLAG_LOG
      )
  ...
  describe "class controls" do
    it "waits any traced task and wraps result in Event" do
      allow(Rptrace::Binding).to receive(:safe_waitpid).with(-1, flags: 7).and_return([5555, 0x2A00])
    ...
    it "spawns tracee and sets default options" do
      event = instance_double(Rptrace::Event, stopped?: true)
      ...
      allow(tracee).to receive(:wait).with(flags: Rptrace::Constants::WALL).and_return(event)
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(
        Rptrace::Constants::PTRACE_SETOPTIONS,
        1010,
        0,
        Rptrace::Constants::PTRACE_O_TRACESYSGOOD
      )
    ...
    it "does not set options when option mask is zero" do
      ...
      allow(tracee).to receive(:wait).with(flags: Rptrace::Constants::WALL).and_return(event)
      expect(Rptrace::Binding).not_to receive(:safe_ptrace).with(
        Rptrace::Constants::PTRACE_SETOPTIONS,
        anything,
        anything,
        anything
    ...
    it "raises when initial stop is not observed on spawn" do
      event = instance_double(Rptrace::Event, stopped?: false, raw_status: 0x1234)
      tracee = instance_double(described_class)
      ...
      end.to raise_error(Rptrace::Error, /did not stop after spawn/)
    end

    it "raises when fork fails" do
      allow(Process).to receive(:fork).and_return(nil)

      expect { described_class.spawn("/bin/true") }.to raise_error(Rptrace::Error, /fork failed/)
    end
    ...
    it "attaches and sets options" do
      ...
      expect(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_ATTACH, 2222, 0, 0)
    ...
    it "raises when initial stop is not observed on attach" do
      event = instance_double(Rptrace::Event, stopped?: false, raw_status: 0x2222)
      tracee = instance_double(described_class)

      allow(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_ATTACH, 3333, 0, 0)
      allow(described_class).to receive(:new).with(3333).and_return(tracee)
      allow(tracee).to receive(:wait).with(flags: Rptrace::Constants::WALL).and_return(event)

      expect do
        described_class.attach(3333)
      end.to raise_error(Rptrace::Error, /did not stop after attach/)
    end

    it "seizes tracee with options" do
      tracee = instance_double(described_class)

      expect(Rptrace::Binding).to receive(:safe_ptrace).with(Rptrace::Constants::PTRACE_SEIZE, 4444, 0, 7)
