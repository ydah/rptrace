...
RSpec.describe Rptrace::Permission do
  describe ".ptrace_scope" do
    it "returns parsed yama ptrace_scope when readable" do
      allow(File).to receive(:readable?).with(Rptrace::Permission::YAMA_PTRACE_SCOPE_PATH).and_return(true)
      allow(File).to receive(:read).with(Rptrace::Permission::YAMA_PTRACE_SCOPE_PATH).and_return("1\n")

      expect(described_class.ptrace_scope).to eq(1)
    end

    it "returns nil when ptrace_scope path is unavailable" do
      allow(File).to receive(:readable?).with(Rptrace::Permission::YAMA_PTRACE_SCOPE_PATH).and_return(false)
  ...
  describe ".effective_capabilities_mask" do
    it "reads CapEff from /proc/self/status" do
      allow(File).to receive(:readable?).with(Rptrace::Permission::PROC_STATUS_PATH).and_return(true)
      allow(File).to receive(:read).with(Rptrace::Permission::PROC_STATUS_PATH).and_return("Name:\truby\nCapEff:\t0000000000000200\n")
  ...
  describe ".cap_sys_ptrace?" do
    it "returns true when CAP_SYS_PTRACE bit is present" do
      allow(described_class).to receive(:effective_capabilities_mask).and_return(1 << Rptrace::Permission::CAP_SYS_PTRACE_BIT)
  ...
  describe ".ensure_privileged!" do
    ...
    it "raises PermissionError with hints when ptrace is not privileged" do
      ...
      end.to raise_error(Rptrace::PermissionError, /ptrace privilege required; run as root; set ptrace_scope <= 1/)
    end
  end
end
